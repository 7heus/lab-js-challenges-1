...
function multiplyBy(numToBeMultiplied, multiplier) {
  multipliedNumbers = [];
  if (numToBeMultiplied.length <= 0) {
    return multipliedNumbers;
  }
  numToBeMultiplied.forEach((num) => {
    multipliedNumbers.push(num * multiplier);
  });
  return multipliedNumbers;
}

multiplyBy(numbers, 6);
...
function filterOut(originalArray, arrayToRemove) {
  ...
  for (i = 0; i < arrayToRemove.length; i++) {
    for (j = 0; j < originalArray.length; j++) {
      if (originalArray[j] === arrayToRemove[i]) {
...
function uniquifyArray(duplicatedArray) {
  let result = [];
  if (duplicatedArray.length === 0) {
    return null;
  }
  for (i = 0; i < duplicatedArray.length; i++) {
    if (result.includes(duplicatedArray[i])) {
      continue;
    } else {
      result.push(duplicatedArray[i]);
    }
  }
  return result;
}
